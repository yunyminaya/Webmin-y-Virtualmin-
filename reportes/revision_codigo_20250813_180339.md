...
Fecha: Wed Aug 13 18:03:39 EDT 2025
...
./instalacion_github_unico.sh:6:# Comando único: curl -sSL https://raw.githubusercontent.com/yunyminaya/Webmin-y-Virtualmin-/main/instalacion_github_unico.sh | sudo bash
./instalacion_github_unico.sh:48:    echo -e "${YELLOW}💡 Uso: curl -sSL https://raw.githubusercontent.com/yunyminaya/Webmin-y-Virtualmin-/main/instalacion_github_unico.sh | sudo bash${NC}"
./instalar.sh:6:# Comando único: curl -sSL https://raw.githubusercontent.com/yunyminaya/Webmin-y-Virtualmin-/master/instalar.sh | sudo bash
...
./orquestador_seguridad_pro.sh:5:#   sudo bash orquestador_seguridad_pro.sh --fix      # Verificar y corregir automáticamente
...
./verificar_actualizaciones.sh:335:        echo -e "   🔄 Ejecuta ${BLUE}sudo ./actualizar_sistema.sh${NC} para actualizar Authentic Theme"
./verificar_actualizaciones.sh:345:        echo -e "   ⚙️  Inicia el servicio Webmin: ${BLUE}sudo systemctl start webmin${NC}"
...
./postfix_validation_functions.sh:116:        echo "💡 Ejecute: sudo apt-get install postfix (Ubuntu/Debian)"
./postfix_validation_functions.sh:117:        echo "💡 Ejecute: sudo yum install postfix (CentOS/RHEL)"
...
./integrar_validaciones_postfix.sh:161:        echo "💡 Ejecute: sudo apt-get install postfix (Ubuntu/Debian)"
./integrar_validaciones_postfix.sh:162:        echo "💡 Ejecute: sudo yum install postfix (CentOS/RHEL)"
...
./verificacion_final_completa_ubuntu_debian.sh:626:    echo -e "   • Para corregir errores: sudo bash remediacion_total_webmin_virtualmin.sh"
./verificacion_final_completa_ubuntu_debian.sh:627:    echo -e "   • Para verificar nuevamente: sudo bash $0"
...
./analisis_codigo_funciones.sh:301:        # Verificar uso de sudo sin validación
...
./analisis_codigo_funciones.sh:306:                log_warning "Uso de sudo sin validación en $(basename "$script_file")"
...
./revision_completa_master.sh:238:   • Reiniciar Webmin: sudo systemctl restart webmin
./revision_completa_master.sh:239:   • Reiniciar Apache: sudo systemctl restart apache2
./revision_completa_master.sh:240:   • Ver logs: sudo tail -f /var/log/webmin/miniserv.log
./instalacion_un_comando.sh:6:# Comando único: curl -sSL https://tu-url/install.sh | sudo bash
...
./verificacion_completa_funciones.sh:748:        echo "   • sudo ./instalacion_completa_automatica.sh  # Instalación completa"
./verificacion_completa_funciones.sh:749:        echo "   • sudo ./verificacion_final_autonomo.sh      # Verificación completa"
./verificacion_completa_funciones.sh:750:        echo "   • sudo systemctl status webmin               # Estado de Webmin"
./verificacion_completa_funciones.sh:751:        echo "   • sudo systemctl status apache2              # Estado de Apache"
./verificacion_completa_funciones.sh:752:        echo "   • sudo ufw status                            # Estado del firewall"
./verificacion_completa_funciones.sh:753:        echo "   • sudo apt update && sudo apt upgrade        # Actualizar sistema"
...
./activar_ssh_macos.sh:104:echo "  3. Verificar configuración: sudo sshd -T"
...
./instalar_webmin_virtualmin.sh:243:rm -rf "$INSTALL_DIR"
...
./tunel_nativo_sin_terceros.sh:681:        rm -rf "$CONFIG_DIR"
...
./verificar_tunel_automatico.sh:336:    echo "🔄 Servicio automático: systemctl start auto-tunnel-manager"
...
./seguridad_avanzada_tunnel.sh:600:    echo "   • systemctl status attack-monitor"
...
./alta_disponibilidad_tunnel.sh:995:    echo "   • systemctl status ha-tunnel-monitor"
...
./completar_seguridad_webmin_virtualmin.sh:276:        sudo systemctl restart apache2 2>/dev/null || true
./completar_seguridad_webmin_virtualmin.sh:294:        sudo systemctl restart httpd 2>/dev/null || true
./corregir_tuneles_100_efectividad.sh:129:# Función para corregir systemctl en macOS
...
./coordinador_sub_agentes.sh:443:    systemctl daemon-reload
./coordinador_sub_agentes.sh:444:    systemctl enable sub-agentes-webmin.service
./coordinador_sub_agentes.sh:447:    log_message "Usar: systemctl {start|stop|status} sub-agentes-webmin"
...
./verificacion_final_completa_ubuntu_debian.sh:145:        if systemctl is-active --quiet "$service" 2>/dev/null; then
./verificacion_final_completa_ubuntu_debian.sh:419:        if systemctl is-active --quiet fail2ban; then
...
./verificar_tunel_automatico_mejorado.sh:464:    echo "   • systemctl status auto-tunnel-manager-v2"
...
./verificador_servicios.sh:37:    if ! systemctl list-unit-files | grep -q "^${service_name}\.service" 2>/dev/null; then
./verificador_servicios.sh:43:    if systemctl is-active --quiet "$service_name" 2>/dev/null; then
./verificador_servicios.sh:68:        if systemctl is-enabled --quiet "$service_name" 2>/dev/null; then
./verificador_servicios.sh:325:if ! systemctl is-active --quiet webmin 2>/dev/null; then
...
./sub_agente_actualizaciones.sh:208:        if systemctl is-enabled --quiet "$service" 2>/dev/null; then
./sub_agente_actualizaciones.sh:209:            if ! systemctl is-active --quiet "$service"; then
./sub_agente_actualizaciones.sh:214:                systemctl restart "$service" >/dev/null 2>&1
./sub_agente_actualizaciones.sh:215:                if systemctl is-active --quiet "$service"; then
./sub_agente_actualizaciones.sh:276:        systemctl --type=service --state=active | grep -E "(webmin|apache|nginx|mysql|postgresql|postfix|ssh)"
./tunel_nativo_sin_terceros.sh:499:    systemctl daemon-reload
./tunel_nativo_sin_terceros.sh:500:    systemctl enable "$SERVICE_NAME"
./tunel_nativo_sin_terceros.sh:601:   - Control de servicio: systemctl status tunel-nativo
./tunel_nativo_sin_terceros.sh:604:   - Iniciar túneles: systemctl start tunel-nativo
./tunel_nativo_sin_terceros.sh:605:   - Detener túneles: systemctl stop tunel-nativo
./tunel_nativo_sin_terceros.sh:606:   - Reiniciar túneles: systemctl restart tunel-nativo
./tunel_nativo_sin_terceros.sh:607:   - Ver estado: systemctl status tunel-nativo
./tunel_nativo_sin_terceros.sh:644:    systemctl start "$SERVICE_NAME"
./tunel_nativo_sin_terceros.sh:653:    log INFO "🔍 Verificar estado: systemctl status $SERVICE_NAME"
./tunel_nativo_sin_terceros.sh:662:        systemctl status "$SERVICE_NAME" 2>/dev/null || echo "Servicio no instalado"
./tunel_nativo_sin_terceros.sh:668:        systemctl stop "$SERVICE_NAME" 2>/dev/null || echo "Servicio no está ejecutándose"
./tunel_nativo_sin_terceros.sh:671:        systemctl start "$SERVICE_NAME" 2>/dev/null || echo "Servicio no instalado"
./tunel_nativo_sin_terceros.sh:674:        systemctl restart "$SERVICE_NAME" 2>/dev/null || echo "Servicio no instalado"
./tunel_nativo_sin_terceros.sh:678:        systemctl stop "$SERVICE_NAME" 2>/dev/null || true
./tunel_nativo_sin_terceros.sh:679:        systemctl disable "$SERVICE_NAME" 2>/dev/null || true
./tunel_nativo_sin_terceros.sh:683:        systemctl daemon-reload
