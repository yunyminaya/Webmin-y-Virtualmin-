...
install_virtualmin_official() {
    log_step "Descargando e instalando Virtualmin con script oficial..."
    ...
    if curl -o virtualmin-install.sh https://software.virtualmin.com/gpl/scripts/virtualmin-install.sh; then
        log_info "Ejecutando instalación oficial de Virtualmin..."
...
install_manual() {
    log_step "Iniciando instalación manual..."
    ...
    if [[ ! -d "${LOCAL_THEME_DIR}" ]] || [[ ! -d "${LOCAL_VIRTUALMIN_DIR}" ]]; then
        log_error "No se encontraron las carpetas authentic-theme-master y/o virtualmin-gpl-master"
        log_info "Asegúrate de tener esos directorios junto a este script"
        exit 1
    fi
    ...
    log_info "Instalando Authentic Theme..."
    if [[ -d "$WEBMIN_DIR/authentic-theme" ]]; then
        log_warning "Authentic Theme ya existe, creando backup..."
    ...
    # Copiar Virtualmin
    log_info "Instalando módulo Virtualmin..."
    if [[ -d "$WEBMIN_DIR/virtual-server" ]]; then
        log_warning "Virtualmin ya existe, creando backup..."
...
configure_webmin() {
    ...
    if [[ -f "/etc/webmin/config" ]]; then
        ...
        log_success "Tema configurado como Authentic Theme"
    fi
    
    # Reiniciar Webmin si está corriendo
    if systemctl is-active --quiet webmin; then
        log_info "Reiniciando Webmin..."
...
main() {
    ...
    case $choice in
        1)
            if check_webmin; then
                log_warning "Webmin ya está instalado. ¿Continuar con Virtualmin? (y/n)"
        ...
        2)
            if ! check_webmin; then
                log_error "Webmin no está instalado. Instálalo primero o usa la opción 1."
                exit 1
            fi
            install_manual
            configure_webmin
    ...
    echo
    log_warning "Nota: Puede ser necesario reiniciar el servidor para que todos los servicios funcionen correctamente."
}
...
echo
log_success "Script completado. ¡Disfruta de tu nuevo panel de control integrado!"
