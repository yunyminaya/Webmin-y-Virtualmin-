...
verify_checksum() {
    ...
    if [[ ! -f "$file" ]]; then
        log_error "Archivo no encontrado para verificacion: $file"
        return 1
    fi
...
handle_error() {
    ...
    log_error "$message"
    log_error "Codigo de error: $error_code"
...
get_system_info() {
    ...
    case "$info_type" in
        ...
        disk)
            ...
            local disk_gb
            disk_gb=$(awk "BEGIN {printf \"%.2f\", $disk_kb / 1024 / 1024}")
...
install_packages() {
    ...
    if [[ -z "$package_manager" ]]; then
        log_error "No se encontro un gestor de paquetes soportado"
        return 1
    fi
