...
check_package_manager() {
    ...
    # Guardar gestor de paquetes en variable global
    PACKAGE_MANAGER="$found_manager"
    ...
    if command -v php &> /dev/null; then
        ...
        if [[ -n "$php_version" ]]; then
            ...
            if [[ "$(printf '%s\n' "$php_version" "7.4.0" | sort -V | head -n1)" != "7.4.0" ]]; then
                ...
                exit $ERROR_PHP_VERSION_TOO_OLD
            fi
        else
    ...
    if [[ -n "$mysql_cmd" ]]; then
        ...
        if [[ -n "$mysql_version" ]]; then
            ...
            if [[ "$(printf '%s\n' "$mysql_version" "5.7.0" | sort -V | head -n1)" != "5.7.0" ]]; then
                ...
                exit $ERROR_MYSQL_VERSION_TOO_OLD
            fi
        else
    ...
    if [[ -n "$apache_cmd" ]]; then
        ...
        if [[ -n "$apache_version" ]]; then
            ...
            if [[ "$(printf '%s\n' "$apache_version" "2.4.0" | sort -V | head -n1)" != "2.4.0" ]]; then
                ...
                exit $ERROR_APACHE_VERSION_TOO_OLD
            fi
        else
    ...
    if command -v php &> /dev/null; then
        ...
        if [[ -n "$php_version" ]]; then
            ...
            if [[ "$(printf '%s\n' "$php_version" "7.4.0" | sort -V | head -n1)" != "7.4.0" ]]; then
                ...
                exit $ERROR_SECURITY_VULNERABILITY
