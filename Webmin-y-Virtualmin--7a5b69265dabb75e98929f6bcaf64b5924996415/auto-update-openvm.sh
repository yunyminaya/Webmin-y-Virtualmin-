...
# Corre via cron cada 5 min. Si detecta nuevo commit, redeploya.
set -e
...
[ "$LATEST_SHA" = "$DEPLOYED_SHA" ] && exit 0  # No changes

log "CHANGE: ${DEPLOYED_SHA:0:8} -> ${LATEST_SHA:0:8} — redeploying"

# Download as regular user (avoid sudo pipe issues with curl)
curl -fsSL "$REPO_RAW/virtualmin-gpl-master/openvm-license-layer.pl" -o /tmp/openvm-license-layer.pl

# Deploy license layer
sudocmd cp /tmp/openvm-license-layer.pl /usr/share/webmin/virtual-server/openvm-license-layer.pl
[ -d /usr/share/webmin/virtual-server/pro ] && \
    sudocmd cp /tmp/openvm-license-layer.pl /usr/share/webmin/virtual-server/pro/openvm-license-layer.pl 2>/dev/null || true
sudocmd chmod 644 /usr/share/webmin/virtual-server/openvm-license-layer.pl
sudocmd chown root:root /usr/share/webmin/virtual-server/openvm-license-layer.pl

# Patch virtual-server-lib.pl if not already done
...
# License file
sudocmd tee /etc/virtualmin-license > /dev/null << 'LICEOF'
...
sudocmd systemctl restart webmin
log "OK"
