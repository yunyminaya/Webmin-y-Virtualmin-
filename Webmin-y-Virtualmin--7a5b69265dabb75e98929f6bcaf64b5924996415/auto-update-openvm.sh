...
LATEST_SHA=$(curl -fsSL "$REPO_API" </dev/null 2>/dev/null | python3 -c "import sys,json;print(json.load(sys.stdin)['sha'])" 2>/dev/null)
...
curl -fsSL "$REPO_RAW/virtualmin-gpl-master/openvm-license-layer.pl" -o /tmp/openvm-license-layer.pl </dev/null
