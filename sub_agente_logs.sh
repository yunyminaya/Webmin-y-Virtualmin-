...
analyze_webmin_logs() {
    ...
    if [ -f "/var/webmin/miniserv.log" ]; then
        local webmin_errors=$(grep "$(date +%d/%b/%Y)" /var/webmin/miniserv.log | grep -i error | wc -l)
        local failed_logins=$(grep "$(date +%d/%b/%Y)" /var/webmin/miniserv.log | grep "Failed login" | wc -l)
        local successful_logins=$(grep "$(date +%d/%b/%Y)" /var/webmin/miniserv.log | grep "Successful login" | wc -l)
        ...
        local suspicious_webmin_ips=$(grep "$(date +%d/%b/%Y)" /var/webmin/miniserv.log | grep "Failed login" | awk '{print $1}' | sort | uniq -c | sort -nr | awk '$1 > 5 {print $2 " (" $1 " intentos)"}')
